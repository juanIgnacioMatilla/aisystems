...
def main(runs=5, test_size=0.25):  # Changed to test_size
    ...
    for run in range(runs):
        ...
        nonlinear_neuron, errors, neurons = nonlinear_perceptron.train(
            X_train, y_train, y_max, y_min, initial_weights, epochs=200
        )
    ...
    print(f"Mean training error last epoch: {np.mean(all_errors[-1])} ± {np.std(all_errors[-1])}")
    print(f"Mean testing error last epoch: {np.mean(all_tests_errors[-1])} ± {np.std(all_tests_errors[-1])}")

    # Calculate mean and std deviation of errors across runs
    ...
    # Define the step
    step = 2
    # Create indices to plot every 20th element
    ...
    plt.title(f'Average SSE across Epochs with Std Dev (75/25 split)')
    plt.xlabel('Epochs')
    plt.ylabel('SSE')
...
if __name__ == "__main__":
    main(runs=5, test_size=0.25)  # Specify the number of runs and test size
