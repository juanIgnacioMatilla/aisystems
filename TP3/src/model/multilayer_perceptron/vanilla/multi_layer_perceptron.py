...
class MultiLayerPerceptron:
    def __init__(
        ...
        self.layers = [
            ...
            for input_size, num_neurons in zip(
                layers_structure[:-1], layers_structure[1:]
            )
        ]
    ...
    def backward_propagate(self, outputs_by_layer, target):
        ...
        gradient = error * self.layers[
            output_layer_index
        ].activation_function_derivative(outputs_by_layer[output_layer_index])
        ...
        for i in reversed(range(len(self.layers) - 1)):
            ...
            weights_transposed = np.transpose(
                [neuron.weights[:-1] for neuron in next_layer.neurons]
            )
            ...
            gradient = error * layer.activation_function_derivative(
                outputs_by_layer[i + 1]
            )
            gradients.append(gradient)
